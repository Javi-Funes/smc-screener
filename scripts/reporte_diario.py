...
# ADRs argentinos — doble cotizacion NYSE + BYMA
ADRS_ARG_NYSE = {
...
def _cargar_ratios():
    ...
    if not os.path.exists(_RATIOS_JSON):
        print(f"  AVISO: {_RATIOS_JSON} no encontrado — usando ratio 1 por defecto")
        return
    ...
    print(f"  Ratios cargados: {len(_RATIOS_CEDEARS)} CEDEARs + {len(_ADRS_ARG_JSON)} ADRs")

def get_ratio(ticker):
    """Ratio desde JSON. Para ADRs usa ADRS_ARG_NYSE como fallback."""
    info = _RATIOS_CEDEARS.get(ticker) or _ADRS_ARG_JSON.get(ticker)
    if info and info.get('ratio'):
        return float(info['ratio'])
    # Fallback: buscar en diccionario hardcodeado de ADRs
...
def calc_fibonacci_pois(sh, sl, price):
    ...
    for nivel in niveles_fib:
        ...
        retrocesos.append({
            'nivel': nivel, 'nombre': nombres_fib[nivel],
...
def generar_reporte(ccl, ccl_fuente, resultados_cedears,
                    resultados_adrs, resultados_byma, rotacion, spy_ret):
    ...
    def format_resultado(r, ccl, es_adr=False, byma_info=None):
        ...
        if fvgs:
            ...
            if fvgs_enc:
                ...
                for f in fvgs_enc[:3]:
                    icono = '🔴' if f['tipo'] == 'BEARISH' else '🟢'
                    lines_r.append(f'    {icono} {f["tipo"]:7}  ${f["low"]:,.2f} — ${f["high"]:,.2f}  (mid: ${f["mid"]:,.2f})  [{f["dist_pct"]:+.1f}%]')
            if fvgs_deb:
                lines_r.append(f'  FVGs DEBAJO (soporte / ordenes de compra):')
                for f in fvgs_deb[:3]:
                    icono = '🟢' if f['tipo'] == 'BULLISH' else '🔴'
                    lines_r.append(f'    {icono} {f["tipo"]:7}  ${f["low"]:,.2f} — ${f["high"]:,.2f}  (mid: ${f["mid"]:,.2f})  [{f["dist_pct"]:+.1f}%]')
        else:
        ...
        if fibs_r:
            ...
            for f in fibs_r:
                ...
                icono  = '🟢' if f['zona'] == 'SOPORTE' else '🔴'
                lines_r.append(f'    {icono} {f["nombre"]:<30}  ${f["precio"]:>10,.2f}  [{f["dist_pct"]:+.1f}%]{star}{actual}')
            lines_r.append(f'  Extensiones (targets de precio):')
            for f in fibs_e:
                lines_r.append(f'    🎯 {f["nombre"]:<30}  ${f["precio"]:>10,.2f}  [{f["dist_pct"]:+.1f}%]')
        ...
        # Precio en pesos con ratio correcto
        if ccl and not r['ticker'].endswith('.BA'):
            ...
            lines_r.append(f'  CCL: ${ccl:,.0f}  |  Ratio: {ratio}:1')
            lines_r.append(f'  Formula: (${r["precio"]:.2f} / {ratio}) x ${ccl:,.0f}')
        ...
        if es_adr and byma_info:
            ...
            lines_r.append(f'  NOTA: Senal confirmada tanto en NYSE como en BYMA')
    ...
    L(f'  (Operas el CEDEAR en pesos. Precio referencia: CCL ${ccl:,.0f})')
    L('-' * 65)
    ...
    if cedears_sin_ob:
        ...
        L('\n  Sin senales Tier 1 hoy en CEDEARs.')
    if cedears_con_ob:
    ...
    L('  (Doble confluencia = senal en NY confirmada en mercado local)')
    L('-' * 65)
    ...
    if adrs_sin_ob:
        for i, r in enumerate(adrs_sin_ob, 1):
            ...
            L(f'\n  #{i}')
            for line in format_resultado(r, ccl, es_adr=True, byma_info=byma_info): L(line)
    else:
        L('\n  Sin senales en ADRs argentinos hoy.')
    if adrs_con_ob:
    ...
    if top3:
        ...
        for i, r in enumerate(top3, 1):
            ...
            L(f'  {i}. {r["ticker"]:6} | Score:{r["score"]}/7 | {r["zona"]} | RSI:{r["rsi"]} | Entrada:${r["precio"]:,.2f}{dc_label}')
    L()
    ...
    if not any([resultados_cedears, resultados_adrs, resultados_byma]):
        L()
        L('  SIN SENALES HOY — El mercado esta fuera de zona de valor.')
...
if __name__ == '__main__':
    ...
    # 0. Cargar ratios desde JSON
    print('Cargando ratios CEDEARs...')
    _cargar_ratios()

    # Universo fijo — 52 CEDEARs con volumen + 11 ADRs + 11 BYMA
